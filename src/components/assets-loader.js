...
export function loadAssets(scene, updateAssetStatus) {
    ...
    /* ------ terrain load ------------ */
    var terrainTask = assetsManager.addMeshTask("terrainLoadTask", "", "./", "morocco.gltf")
    ...
    terrainTask.onSuccess = function(task) {
      ...
      newMeshes[0].position = new  BABYLON.Vector3(-0,1.5,.5)
...
export function addGround(scene) {
  ...
  function addNewTerrain(scene) {
     ...
     BABYLON.SceneLoader.ImportMesh("", "./", "morocco.gltf", scene, function (newMeshes) {;
